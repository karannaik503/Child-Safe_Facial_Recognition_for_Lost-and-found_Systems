...
class ChildSafetyApp(tk.Tk):
    ...
    def manual_cleanup(self):
        ...
        confirm_dialog.geometry("400x230")
        ...
        ttk.Label(
            confirm_dialog, 
            text="This will permanently delete closed cases.\nWhat would you like to do?",
            font=("Arial", 12)
        ).pack(pady=(20, 10))
        
        # Add options frame
        options_frame = ttk.Frame(confirm_dialog)
        options_frame.pack(pady=(0, 10))
        
        # Option variables
        cleanup_option = tk.StringVar(value="older")
        days_var = tk.StringVar(value="30")
        
        # Add radio buttons for options
        older_radio = ttk.Radiobutton(
            options_frame, 
            text="Delete cases older than:", 
            value="older", 
            variable=cleanup_option
        )
        older_radio.grid(row=0, column=0, sticky="w", padx=5, pady=5)
        
        days_entry = ttk.Entry(options_frame, textvariable=days_var, width=5)
        days_entry.grid(row=0, column=1, padx=5, pady=5)
        
        ttk.Label(options_frame, text="days").grid(row=0, column=2, padx=5, pady=5)
        
        # Add option to delete all closed cases immediately
        all_radio = ttk.Radiobutton(
            options_frame, 
            text="Delete ALL closed cases immediately", 
            value="all", 
            variable=cleanup_option
        )
        all_radio.grid(row=1, column=0, columnspan=3, sticky="w", padx=5, pady=5)
        ...
        def on_confirm():
            try:
                if cleanup_option.get() == "older":
                    days = int(days_var.get())
                    if days < 0:
                        messagebox.showerror("Error", "Days must be at least 0")
                        return
                    ...
                    confirm_dialog.destroy()
                    deleted_count = cleanup_closed_cases(older_than_days=0)
                    messagebox.showinfo("Cleanup Complete", f"{deleted_count} closed cases were permanently deleted")
            except ValueError:
    ...
    def close_searched_case(self):
        ...
        try:
            ...
            # Create closure options dialog
            options_dialog = tk.Toplevel(self)
            options_dialog.title("Close Case Options")
            ...
            ttk.Label(
                options_dialog,
                text="How would you like to close this case?",
                font=("Arial", 12)
            ).pack(pady=(20, 10))
            
            # Option variable
            close_option = tk.StringVar(value="regular")
            
            # Add radio buttons for options
            regular_radio = ttk.Radiobutton(
                options_dialog,
                text="Mark as closed (keep data for 30 days)",
                value="regular",
                variable=close_option
            )
            regular_radio.pack(anchor="w", padx=20, pady=5)
            
            immediate_radio = ttk.Radiobutton(
                options_dialog,
                text="Delete case immediately (permanent)",
                value="immediate",
                variable=close_option
            )
            immediate_radio.pack(anchor="w", padx=20, pady=5)
            
            # Buttons frame
            buttons_frame = ttk.Frame(options_dialog)
            buttons_frame.pack(pady=20)
            
            def on_close_confirm():
                option = close_option.get()
                ...
                # Close the case based on selected option
                if option == "immediate":
                    # Confirm immediate deletion
                    confirm = messagebox.askyesno(
                        "Confirm Immediate Deletion",
                        "This will PERMANENTLY DELETE all data for this case immediately.\n"
                    ...
                    if update_case_status(embedding_id, "Closed", delete_after_days=0):
                        messagebox.showinfo("Success", "Case closed and deleted successfully")
                        # Clear the case details
                        ...
                        messagebox.showerror("Error", "Failed to close and delete case")
                else:
                    # Regular case closure
    ...
    def close_identified_case(self):
        ...
        try:
            ...
            # Create closure options dialog
            options_dialog = tk.Toplevel(self)
            options_dialog.title("Close Cases Options")
            options_dialog.geometry("400x200")
            options_dialog.transient(self)
            options_dialog.grab_set()
            ...
            ttk.Label(
                options_dialog,
                text=f"How would you like to close {len(embedding_ids)} identified cases?",
                font=("Arial", 12)
            ).pack(pady=(20, 10))
            ...
            immediate_radio = ttk.Radiobutton(
                options_dialog,
                text="Delete cases immediately (permanent)",
                value="immediate",
                variable=close_option
            )
            immediate_radio.pack(anchor="w", padx=20, pady=5)
            ...
            def on_close_confirm():
                ...
                # If immediate deletion, get additional confirmation
                if option == "immediate":
                    confirm = messagebox.askyesno(
                        "Confirm Immediate Deletion",
                        f"This will PERMANENTLY DELETE all data for {len(embedding_ids)} cases immediately.\n"
                        "This action cannot be undone. Are you sure?",
                        icon=messagebox.WARNING
                    )
                ...
                # Close all the cases
                success_count = 0
                delete_after_days = 0 if option == "immediate" else 30
                
                for eid in embedding_ids:
                    child = get_child_by_embedding_id(int(eid))
                    if child and child['case_status'] == "Open":
                        if update_case_status(eid, "Closed", delete_after_days=delete_after_days):
                            success_count += 1
                
                if success_count > 0:
                    messagebox.showinfo(
                        "Success", 
                        f"Successfully closed {success_count} case(s)" +
                        (" and deleted permanently" if option == "immediate" else " and scheduled for deletion")
                    )
